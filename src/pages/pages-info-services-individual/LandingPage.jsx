...
function LandingPage() {
  ...
  const dataType =
    dataTypePages[pathname] || dataTypePages["/services/landing-page"];
      ...
      <section className="container-info">
        <div className="hero-section-landing">
          <div className="text-hero-block">
            ...
            <p className="subtitle-hero">{dataType["subtitle-hero"]}</p>
        ...
        <div className="landing-info">
          <div className="landing-description">
            <h2>
              <span>{dataType["h2-1-span-landing-description"]}</span>
              {dataType["h2-1-landing-description"]}
            </h2>
            <h2>
            <span>{dataType["h2-2-span-landing-description"]}</span>
            {dataType["h2-2-landing-description"]}
            </h2>
          </div>
          ...
          <div className="landing-benefits">
            ...
            <ul className="landing-benefits-list">
              {dataType["top-li-span-landing-benefits-list"].map(
                (item, index) => (
                  <li key={index}>
                    <span>{item}</span>
                    {dataType["top-li-landing-benefits-list"][index]}
                  </li>
                )
              )}
            </ul>
          ...
          {dataType["bottom-li-landing-benefits-list"] ? (
            <div className="landing-benefits">
              <h3>{dataType["h3-landing-benefits"]}</h3>

              <p className="landing-benefits-list-p">
                {dataType["landing-benefits-list-p"]}
              </p>
              <ul className="landing-benefits-list">
                {dataType["bottom-li-span-landing-benefits-list"].map(
                  (item, index) => (
                    <li key={index}>
                      <span>{item}</span>
                      {dataType["bottom-li-landing-benefits-list"][index]}
                    </li>
                  )
                )}
              </ul>
